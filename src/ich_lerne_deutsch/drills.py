...
class Flashcards(Drill):
    ...
    def next(self, event=None):
        if not self._window_is_dead:
            if self.current_side == 1:
                ...
                out = self.text_rehearsal_flashcard_side1()
                self._canvas_items.extend(out)
                self.current_side = 1
    ...
    def text_rehearsal_flashcard_side1(self):
        ...
        if isinstance(w,word_classes.Noun):
            ...
            definition = self._canvas.create_text(400, 63, text=w._definition, font=("Ariel", 20, "bold"),
                                                  tags="definition",
                                                  justify="center")
            ...
            if w.perfekt_hilfsverb=='haben':
                hv = 'hat'
            elif w.perfekt_hilfsverb=='sein':
                hv = 'ist'
            elif w.perfekt_hilfsverb in ['sein/haben','haben/sein']:
                hv = 'hat/ist'

            perfekt_text = 'Es '+hv +' '+ w.partizip_2
            self._canvas.itemconfig("perfekt", text=perfekt_text)
            return [definition]
        elif isinstance(w, word_classes.Word):
            definition = self._canvas.create_text(400, 63, text=w._definition, font=("Ariel", 20, "bold"),
                                                  tags="definition",
                                                  justify="center")
            return [definition]
    def text_rehearsal_flashcard_side0(self):
        ...
        elif isinstance(self._current_word,word_classes.Word):
            texts.append(
                canvas.create_text(100, 163, text=self._current_word._root_form, font=("Ariel", 60, "bold"),
                                   tags="inf", justify="right", anchor="w"))
        return texts
...
class NounArticles(Test):
    ...
    def set_up_question_card(self, event=None):
        ...
        instr_text = 'Select the correct definite article.\n Case = NOM.'
    ...
    @staticmethod
    def filter_word_list(list):
        out_list = [item for item in list if isinstance(item, word_classes.Noun)]
        out_list = [item for item in out_list if all([item._only_plural==False,item.gender!='NOG'])]
        return out_list
...
class PluralForms(WritingTest):
    ...
    @staticmethod
    def filter_word_list(list):
        def has_singular(word):
            if word._root_form is None:
                return False
            if len(word._root_form)==0:
                return False
            return True
        def has_plural(word):
            if word._plural_form is None:
                return False
            if len(word._plural_form)==0:
                return False
            return True
        out_list = [item for item in list if isinstance(item, word_classes.Noun)]
        out_list = [item for item in out_list if all([has_plural(item),has_singular(item)])]
        return out_list
...
class PerfektForms(WritingTest):
    ...
    def set_up_question_card(self, event=None):
        ...
        if self._current_word.perfekt_hilfsverb == 'haben':
            hv = 'hat'
        elif self._current_word.perfekt_hilfsverb == 'sein':
            hv = 'ist'
        elif self._current_word.perfekt_hilfsverb in ['sein/haben', 'haben/sein']:
            hv = 'hat/ist'

        perfekt_text = 'Es ' + hv + ' ' + self._current_word.partizip_2
    ...
    @staticmethod
    def filter_word_list(list):
        def has_perfekt(word):
            if word.partizip_2 is None:
                return False
            if len(word.partizip_2)==0:
                return False
            if word.perfekt_hilfsverb not in ['haben', 'sein']:
                return False
            return True
        out_list = [item for item in list if isinstance(item, word_classes.Verb)]
        out_list = [item for item in out_list if has_perfekt(item)]
        return out_list
...
class GermanWordMeanings(WritingTest):
    ...
    def set_up_question_card(self,event=None):
        ...
        self._canvas_items.append(
            self._canvas.create_text(self._canvas_dimensions[0] / 2, self._canvas_dimensions[1]/3, text=definition, font=("Ariel", 30, "bold"),
    ...
    @staticmethod
    def filter_word_list(list):
        def has_definition_and_word(item):
            if (item._definition is None) | (len(item._definition)==0):
                return False
            if isinstance(item,word_classes.Noun):
                if item._only_plural:
                    if (item._plural_form is None) | (len(item._plural_form) == 0):
                        return False
            elif isinstance(item,word_classes.Verb):
                if (item.infinitive_form is None) | (len(item.infinitive_form) == 0):
                    return False
            else:
                if (item._root_form is None) | (len(item._root_form) == 0):
                    return False
            return True
        out_list = [item for item in list if has_definition_and_word(item)]
        return out_list
